...
add tag by hlq:
1、http框架处理请求的第一步
2、nginx收到用户发起tcp连接请求
3、事件框架负责建立tcp连接
4、连接建立成功后，http框架介入处理
5、read handle is a\ngx_http_wait_request_handler\ngx_http_v2_init\ngx_http_ssl_handshake
*/
void
ngx_http_init_connection(ngx_connection_t *c)
{
	...
	/*
	add tag by hlq:add an event to epoll
	*/
...
/*
add tag by hlq
1、构造ngx_http_request_t
2、修改读事件为ngx_http_process_request_line
3、调用ngx_http_process_request_line解析请求行
*/
static void
...
/*
add tag by hlq
1、解析请求头，放在r->headers_in
2、由Host头调用ngx_http_find_virtual_server找到server配置块ngx_http_core_srv_conf_t
3、r->srv_conf、r->loc_conf被重新设置
*/
static void
...
/*
add tag by hlq
1、设置read handle和write handle为ngx_http_request_handler
2、根据internal(rewrite标志)设置r->phase->handler
3、执行ngx_http_core_run_phases执行各http模块功能
4、run posted
*/
void
...
/*
add tag by hlq
1、调用ngx_http_core_run_phases继续调用http其它模块功能
*/
static void
